...
function getVolumeDiscountState(tiers: VolumeDiscountTier[], subtotal: number) {
  ...
  const progressPercent =
    range > 0 ? Math.min(100, Math.max(0, ((subtotal - prevThreshold) / range) * 100)) : 100;
  ...
  return {
    ...
    extraPercent: nextTier ? nextTier.discountPercent - (activeTier?.discountPercent ?? 0) : 0,
    progressPercent,
  };
}
    ...
    <div className="flex flex-col gap-2">
      {showProgress && vol && (
        ...
        >
          ...
          >
            Add ${vol.amountToNext.toFixed(2)} to save an extra {vol.extraPercent}% on your order
          </span>
        </div>
      )}
        ...
        >
          <div
            ...
            style={{
              width: "100%",
              background:
                "linear-gradient(90deg, rgba(26,173,25,0.3) 0%, rgba(26,173,25,0.1) 100%)",
            }}
          />
          <span
...
export function OrderSummary({
  ...
  return (
    <>
      <FastCheckoutPanel
        isOpen={fastCheckoutOpen}
        onClose={() => setFastCheckoutOpen(false)}
        gameName={title}
        estimatedTime={estimatedTime}
        startIn={startInLabel}
        summaryRows={rows}
        totalAmount={totalAmount}
        priceRows={finalRows.map((r) => ({
          label: r.label,
          value: r.value,
          color: /free/i.test(r.value)
            ? ("green" as const)
      ...
      >
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              >
                ...
                <span className="mt-1 block font-body text-xs leading-5 text-white/80">
                  ...
                  <span className="font-bold text-white">{startInLabel}</span>, depending on queue
                  and booster availability.
                </span>
              </InfoTooltip>
            </div>
              ...
              {extras.map((opt, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`font-body text-sm font-semibold ${valueColorClass(opt.cost)}`}
                    >
                      {opt.cost}
                    </span>
                  </div>
        ...
        <p className="mt-4 font-body text-xs leading-relaxed text-white/60">
          By placing an order at <span className="font-bold text-white/80">ogedge.com</span>{" "}
          you&rsquo;re agreeing to our <span className="font-bold text-white/80">Terms of Use</span>{" "}
          and <span className="font-bold text-white/80">Privacy Policy</span>
        </p>
