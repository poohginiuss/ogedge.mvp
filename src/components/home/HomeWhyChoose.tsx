...
export function HomeWhyChoose() {
  return (
    <section className="relative w-full overflow-hidden bg-dark-main">
          ...
          <div className="mt-10 grid grid-cols-2 gap-x-6 gap-y-10">
            {features.map((feature) => (
              <div key={feature.id} className="flex flex-col items-center text-center">
                ...
                <p className="mt-1 font-body text-sm leading-5 text-white">{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Desktop layout */}
      <div className="mx-auto hidden w-full max-w-[1440px] items-center gap-12 px-6 py-20 md:px-12 lg:flex lg:px-[60px] lg:py-[120px]">
        <div className="flex w-[55%] flex-col gap-16">
            ...
            {features.map((feature) => (
              <div key={feature.id} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-0.5">
                  <h3 className="font-body text-2xl font-bold leading-8 text-brand-main">
                    {feature.title}
                  </h3>
                  <p className="font-body text-lg leading-7 text-white">{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="relative flex w-[45%] items-center justify-center">
          <div
            className="absolute right-0 top-1/2 h-[390px] w-[250px] -translate-y-1/2 rounded-full"
          ...
          <Image
            ...
            height={621}
            sizes="45vw"
            className="relative h-auto w-full max-w-[790px] object-contain"
          />
          <div
            className="pointer-events-none absolute bottom-0 left-0 h-[172px] w-full"
